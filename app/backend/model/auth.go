...
func (auth AuthModel) GetAuth() *AuthModel{
	return authModel
}
...
type TokenDetails struct {
	...
	AtExpires    int64
	RtExpires    int64
	Role         int64 //0 manager, 1 : sale, 2 employee
}
...
func (authM AuthModel) FetchAuth(authD *AccessDetails) (int64, error) {
	userid, err := redis.GetRedisClient().Get(authD.AccessUUID).Result()
	if err != nil {
		return 0, err
	}
...
func (authM AuthModel) DeleteAuth(givenUUID string) (int64, error) {
	deleted, err := redis.GetRedisClient().Del(givenUUID).Result()
	if err != nil {
		return 0, err
	}
